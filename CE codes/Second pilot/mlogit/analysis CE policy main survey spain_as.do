...
use "Data\Main survey Spain\CE policy and backgroundvars.dta", clear
...
esttab using "Tables\Descriptive\treatment_covar.rtf", replace cells(mean(fmt(2))) label mtitles("Control" "Info 1" "Info 2" "Info 3" "Info 4") collabels(, none) fonttbl(\f0\fnil Arial; )
...
tabstat choice_policy if alt==1&d_madrid==1, by(treatment) stat(mean sd count)

prtest choice_policy if alt==1&d_madrid==1&(treatment_num==1|treatment_num==2), by(treatment) cluster(participantcode) rho(0.5)
